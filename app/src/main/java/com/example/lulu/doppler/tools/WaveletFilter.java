...
public class WaveletFilter {
    ...
    public static short[] filter(short[] buff) {
        ...
        for (int j = level-1; j >= 0; j--){
            ...
            if(j==3) a=buffer; else a=buffer_reconstruit;
    ...
    public static List<Double> decomposition(List<Double> buff, double HI_D [], double LO_D[]) {
    ...
    static List<Double> reconstruction(List<Double> buff_low, List<Double> buff_high, double HI_D [], double LO_D[], int current_size) {
        ...
        int nb_val = Math.round(((tmp_lo.length-1)-current_size)/2);
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            System.out.println("la valeur de result vaut :" + result[i]);
        }
